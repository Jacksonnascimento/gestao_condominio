...
public class FinanceiroCobrancaController {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAuthority('ROLE_GLOBAL_ADMIN') or @financeiroCobrancaService.hasPermissionToManageCobrancaByCobrancaId(#id)")
    ...
    @PostMapping("/gerar-individual")
    @PreAuthorize("hasAuthority('ROLE_GLOBAL_ADMIN') or @financeiroCobrancaService.hasPermissionToManageCobrancaByUnidadeId(#requestDTO.unidadeId)")
    public ResponseEntity<?> gerarCobrancaIndividual(@RequestBody GerarCobrancaIndividualRequestDTO requestDTO) {
        try {
            FinanceiroCobranca novaCobranca = financeiroCobrancaService.gerarCobrancaIndividual(requestDTO);
    ...
    @PreAuthorize("hasAuthority('ROLE_GLOBAL_ADMIN') or @financeiroCobrancaService.hasPermissionToManageCobrancaByCondominioId(#requestDTO.condominioId)")
    public ResponseEntity<?> gerarCobrancaEmLote(@RequestBody GerarCobrancaLoteRequestDTO requestDTO) {
        try {
            financeiroCobrancaService.gerarCobrancasEmLote(requestDTO);
            return new ResponseEntity<>("Processo de geração em lote iniciado.", HttpStatus.ACCEPTED);
        } catch (Exception e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.BAD_REQUEST);
        }
    }
